...
class PointPillarRebuttalLoss(nn.Module):
    def __init__(self, args):
        super(PointPillarRebuttalLoss, self).__init__()
    ...
    def forward(self, output_dict, target_dict, is_truck):
        """
        Parameters
        ----------
        output_dict : dict
        ...
        """
        if is_truck:
            rm = output_dict['rm_truck']
            psm = output_dict['psm_truck']
        else:
            rm = output_dict['rm']
            psm = output_dict['psm']
    ...
    def logging(self, epoch, batch_id, batch_len, writer, pbar=None):
        ...
        log_msg = (f"[Epoch: {epoch:d}][Step: {batch_id+1:d}/{batch_len:d}] "
                   f"Loss: {total_loss.item():.4f} || "
                   f"Conf Loss: {conf_loss.item():.4f} || "
                   f"Loc Loss: {reg_loss.item():.4f}")
        print(log_msg) if pbar is None else pbar.set_description(log_msg)
