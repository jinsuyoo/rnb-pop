...
from opencood.utils import common_utils, yaml_utils
...
def calculate_tp_fp(det_boxes, det_score, gt_boxes, result_stat, iou_thresh,
    ...
    if det_boxes is not None and gt_boxes is not None:
    ...
    elif det_boxes is None:
        #gt = gt_boxes.shape[0]
        if gt_boxes is not None:
            gt = gt_boxes.shape[0]
        else:
            gt = 0
    elif gt_boxes is None:
        for _ in range(det_boxes.shape[0]):
            fp.append(1)
            tp.append(0)
        gt = 0
...
def calculate_ap(result_stat, iou):
    ...
    return ap, mrec, mprec, rec


def eval_final_results(result_stat, save_path, iou_thresh, range=""):
    msg = f"The range is {range}\n"
    dump_dict = {}
    file_name = 'eval.yaml' if range == "" else range+'_eval.yaml'
    for cur_thresh in iou_thresh:
        recall = sum(result_stat[cur_thresh]['tp']) / result_stat[cur_thresh]['gt']
        precision = sum(result_stat[cur_thresh]['tp']) / (sum(result_stat[cur_thresh]['tp']) + sum(result_stat[cur_thresh]['fp']))

        ap, _, _, _ = calculate_ap(result_stat, cur_thresh)
        dump_dict.update({
            'ap_%s' % cur_thresh: ap,
            'recall_%s' % cur_thresh: recall,
            'precision_%s' % cur_thresh: precision,
        })
        yaml_utils.save_yaml(dump_dict, os.path.join(save_path, file_name))
        msg += f'IoU {cur_thresh} AP/Recall/Precision: {ap:.4f}/{recall:.4f}/{precision:.4f}\n'
