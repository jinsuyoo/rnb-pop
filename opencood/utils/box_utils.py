...
def mask_boxes_outside_range_numpy(boxes, limit_range, order,
    ...
    assert boxes.shape[1] == 8 or boxes.shape[1] == 7, print(boxes.shape)

    new_boxes = boxes.copy()
    if boxes.shape[1] == 8:
        new_boxes_label = new_boxes[:, -1]
        new_boxes= new_boxes[:, :-1]
    
    # NOTE: I guess previously intended for [8, 3]
    #if boxes.shape[1] == 7:
...
def mask_boxes_outside_range_and_num_points_numpy(
    boxes, pcd, limit_range, order, min_num_corners=2, min_num_points=5):
    """
    Parameters
    ----------
    boxes: np.ndarray
    ...
    # check the number of points in the bounding box
    mask_points = np.zeros(mask.shape, dtype=bool)
    for i, box in enumerate(new_boxes):
        mask_points[i] = get_points_in_rotated_box_3d(pcd[:, :3], box).shape[0] >= min_num_points

    final_mask = np.logical_and(mask, mask_points)

    return boxes[final_mask], final_mask
...
def project_world_objects(
    object_dict, output_dict, transformation_matrix, lidar_range, order, 
    pseudo_label_path='', use_pseudo_label=False, npy_label_order=None, detector='pointpillar', specific_object=None):
    """
    ...
    if use_pseudo_label:
        # NOTE: always assume lwh
        pseudo_label = np.load(pseudo_label_path) # (num_boxes x 7)
        if pseudo_label.ndim == 3:
            pseudo_label = pseudo_label.squeeze(1)

        if npy_label_order == 'lwh': # v2v4real only support 'hwl'
            if detector != 'pixor': # pixor only supports 'lwh
                pseudo_label = pseudo_label[:, [0, 1, 2, 5, 4, 3, 6]]
        #else:
        #    raise NotImplementedError()
        #pseudo_label = project_box3d(pseudo_label, transformation_matrix) # already projected
        if pseudo_label.shape[0] != 0: # if box exists
            #pseudo_label = project_box3d(pseudo_label, transformation_matrix)
            for object_id, bbx in enumerate(pseudo_label):
                ass_id = object_id
                #print('my', bbx_lidar.shape)
                #t_matrix = np.linalg.inv(transformation_matrix)
                ...
                #bbx_lidar = np.dot(t_matrix, bbx).T
                #print(bbx_lidar.shape)
                #bbx_lidar = bbx
                #bbx_lidar = np.expand_dims(bbx_lidar, 0)
                ...
                #    output_dict.update({object_id: {'coord': bbx_lidar, 'ass_id': ass_id}})
                output_dict.update({object_id: {'coord': bbx[None], 'ass_id': ass_id}})
        ...
        for object_id, object_content in object_dict.items():
            ...
            if specific_object is not None and obj_type != specific_object:
                continue
            ...
            #print('111', bbx_lidar.shape)
            bbx_lidar = np.expand_dims(bbx_lidar[:, :3], 0)
            #print('222', bbx_lidar.shape)
            bbx_lidar = corner_to_center(bbx_lidar, order=order)
            #print('333', bbx_lidar.shape)
