...
def pcd_to_np(pcd_file, concat_intensity=True):
    """
    ...
    if concat_intensity:
        ...
        pcd_np = np.concatenate([xyz, np.zeros([xyz.shape[0], 1])], axis=1)
    
    assert pcd_np.shape == (xyz.shape[0], 4)

    return np.asarray(pcd_np, dtype=np.float32)


def mask_points_by_range(points, limit_range, ground=None):
    """
...
def mask_ego_points(points, ground=None):
    """
    ...
    if ground is not None:
        ground = ground[np.logical_not(mask)]

    return points, ground


def shuffle_points(points, ground=None):
    shuffle_idx = np.random.permutation(points.shape[0])
    points = points[shuffle_idx]
    if ground is not None:
        ground = ground[shuffle_idx]
