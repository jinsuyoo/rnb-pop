...
def load_saved_model(saved_path, model, load_epoch=None):
    """
...
def setup_train(hypes, use_time=True):
    """
...
def create_log_folder(save_path, hypes, use_time=True):
    """
    Create folder for saved model based on current timestep and model name
    ...
    if use_time:
        folder_name = current_time.strftime("_%Y_%m_%d_%H_%M_%S")
        folder_name = save_path + folder_name
    else:
